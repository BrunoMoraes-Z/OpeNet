...
export default class UserController {
  ...
  public async create(request: Request, response: Response): Promise<Response> {
    const { first_name, last_name, email, curso, periodo, dt_nascimento, password, g_id } = request.body;
    const service = new CreateUserService();
    const splitted = dt_nascimento.split('/');
    const dt_nasc = new Date(`${splitted[1]}/${splitted[0]}/${splitted[2]}`);

    const user = await service.execute({ first_name, last_name, email, curso, periodo, dt_nasc, password, g_id });
